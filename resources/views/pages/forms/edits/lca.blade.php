<form action="{{ route('update.lca',encrypt($all_form->id)) }}" method="POST" id="update_lca" enctype="multipart/form-data">
    <div class="card-body">
        @csrf          
        <div class="row">
        ...
        <input type="hidden" name="rca_form_id"  value="{{ $all_form->model->rca_form->id }}">

        </div>  
         <div class="card">
            <div class="card-header">
                <h4><b>Cash Advance [{{ $all_form->model->rca_form->control_number }}] Details</b></h4>
            </div>
            <div class="card-body">
                <div class="row mb-3">
                    <div class="col-lg-6">
                        <h4>Name: <b>{{ ($all_form->model->rca_form->name ?? '' )}}</b></h4>
                        <h4>Department: <b>{{ ($all_form->model->rca_form->department->name ?? '' )}}</b></h4>
                    ...
                    <div class="col-lg-6 text-right">
                        <div class="table-responsive">
                            <table class="table table-bordered text-center">
                                ...
                                <tbody>
                                    @foreach($all_form->model->rca_form->rca_form_item()->get() as $index => $item)
                                        <tr>
                                            <td>{{ $index + 1 }}</td>
                        ...
                        <h2>Total Budget of Cash Advance: <b>₱{{  number_format($all_form->model->rca_form->total_amount ?? 0.00 , 2) }}</b></h2>
                        <input type="hidden" name="rca_amount" value="{{ $all_form->model->rca_form->total_amount }}"> 
            ...
            <div class="col-md-12">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered text-center" id="dynamicTable">
                        ...
                        @endphp
                        <tbody >
                            @foreach ($all_form->model->lca_form_item()->get() as $index => $item)
                            <tr>
                                <td class="row-number">1</td>       
                                <td><input type="date" name="items[{{ $index }}][date]" value="{{ $item['date'] }}" class="form-control text-center date"/></td>
                                <td><input type="text" name="items[{{ $index }}][desc]" value="{{ $item['item_description'] }}" class="form-control text-center desc" /></td>             
                                <td><input type="text" name="items[{{ $index }}][area]" value="{{ $item['area'] }}" class="form-control text-center area" /></td>
    ...
    <div class="card-footer text-right">
        <input type="hidden" id="status" name="status" form="update_lca" value="pending">
