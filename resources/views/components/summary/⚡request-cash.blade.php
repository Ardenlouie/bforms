...
new class extends Component
{
    ...
    private function generateControlNumber() {
        $date_code = date('Y');
        do {
            $control_number = 'RCA-0'.$this->company_id.'-'.$date_code.'-001';

            $rca = RequestCash::withTrashed()->orderBy('control_number', 'DESC')->where('company_id', $this->company_id)->first();
            ...
            if(!empty($rca->control_number)) {
                ...
                $control_number = "RCA-0$this->company_id-$date_code-$formatted_number";
            }

        } while(RequestCash::withTrashed()->where('control_number', $control_number)->where('company_id', $this->company_id)->exists());
